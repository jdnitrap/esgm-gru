...
| Oja's rule (implemented, properly bundled with its own eta, and measured — see `learning_rules.py` and `EXPERIMENT_LOG.md`) | Oja, E. (1982), "A Simplified Neuron Model as a Principal Component Analyzer," *J. Mathematical Biology* |
...
- **`learning_rules.py`** — the edge weight-update formula, pluggable
  ...
  pre-refactor formula. Each rule owns its own tuning constants
  (`eta`/`lam`) and an optional `normalize()` post-update hook, rather
  than borrowing the graph's — a real gap the user found (session 5):
  the first version passed the graph's own `eta`/`lam` into whichever
  rule was plugged in, and ran Hebbian's own external normalization
  step regardless of which rule was active, so "swapping" a rule
  wasn't actually testing that rule on its own terms. `OjaLearning`
  (Oja, 1982), corrected this way and given its own tuned `eta`, needed
  neither the non-negativity floor nor any external rescue mechanism
  at all on the real graph, while Hebbian's own `normalize()` fired
  31,234 times over the same run — the opposite conclusion from the
  first, flawed comparison. Still kept as a tested option, not switched
  to be the default, but now purely because every other tuned constant
  in this system was calibrated against Hebbian, not because Oja
  underperformed. See `EXPERIMENT_LOG.md` for both the flawed and the
